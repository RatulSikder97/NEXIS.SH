...
The captures that follow are ordered the way a new organisation would encounter the platform.

### 5.2.1 Authentication and Onboarding

Access relies on a local JSON Web Token (JWT) session, kept in an HttpOnly cookie, with optional WorkOS [12] Single Sign-On available on top of it. The sign-up screen (Figure 5.1) creates the account, and the forgot-password flow issues a 32-byte reset token, SHA-256-hashed with a 30-minute time-to-live; it returns the same 202 response regardless of whether the e-mail address actually exists, hence no account information leaks out.
...
Once the administrator logs in, the console home (Figure 5.2) presents KPI tiles covering Mean Time To Recovery (MTTR), recovery success rate, open incidents, and mean tokens per run; alongside these sit an incidents-over-time chart, a system status panel, a projects health grid, and an activity feed.
...
Moving to the Agents page (Figure 5.3), the nine-agent fleet is listed across both layers: the five Execution Team agents (Architect, Backend, QA, DevOps, Data Engineer) and the Self-Healing Loop agents (Sentinel, Pathfinder, Synthesiser, Validator). Every ingested incident, in turn, appears on the Incidents page (Figure 5.4).
...
Under Settings sit member management with `owner` / `admin` / `member` roles, a genuine heuristic role-recommendation panel offering evidence-backed rationales with a 30-day cool-down on dismissal, remotely revocable sessions, scoped API keys (`nx_live_...`), and Stripe billing [13].

### 5.2.3 Self-Healing Loop — Live Evidence

This subsection carries the evidentiary weight: two real, end-to-end executions of the nine-agent RecoveryPipeline, run against a live local LLM (Ollama, `qwen2.5-coder:14b` and `gpt-oss:20b`), are shown exactly as they occurred, one safely auto-rejected at the approval gate and the other approved and fully succeeded. The Live Demo entry point offers 27 fault-scenario cards, of which exactly one is fully wired end-to-end today; the rest remain UI-complete but backend-pending, each labelled "Coming soon". Both runs used the wired path, and Figure 5.5 shows the pipeline structure.
...
Every step then completed green, leaving severity at "Medium" and approval at "Pending" inside a 120-second window. That window elapsed with no decision, and the workflow terminated as `timeout_rejected` — an honest negative result, reported deliberately: absent a human decision, the system refuses to deploy unreviewed code rather than proceeding.

The second run supplies the positive counterpart to this result: approved within the window, it records "Medium" severity, "Approved", "Succeeded", and a full nine-step timeline completing in 1 minute 40 seconds, with real per-agent token and cost figures throughout (Figure 5.7).
...
Pending runs surface in the Approvals queue (Figure 5.8), where three human actions are available (Reject, Modify, Approve), each decided in the dialog shown in Figure 5.9. Modify is where the Reinforcement Learning from Human Feedback (RLHF) mechanism becomes concrete: the engineer edits the LLM-generated unified diff before approving it, and that edited diff, along with every terminal decision, is persisted to `feedback_examples`, feeding a JSONL export for future fine-tuning. Submitting a paid fine-tuning job, however, remains a deliberate manual step rather than an automated one.
...
The patch itself is genuinely model-generated: the Backend step's raw JSON records the unified diff produced by `qwen2.5-coder:14b` with `provider="ollama"`, parsed model output, not a fixture template. The Backend agent's own detail page appears in Figure 5.10.
...
Deploy action, live status, preview URL, logs, and history all live on the project's Ops tab, which fronts the preview-deploy engine. Attempting Deploy on a project with no GitHub App connection (Figure 5.11) returned the real validation error "project has no github repository bound", surfaced verbatim. This is *not* a successful deploy; rather, it is the validation chain correctly rejecting a request it cannot fulfil, and the limitation is environmental (no GitHub App credentials in the evaluation environment) rather than a defect in the engine.
...
A failed build or a failed health check inserts a real incident, with `source: "deploy_engine"`, into the very same Sentinel-to-RecoveryPipeline path used by every other incident source.

### 5.2.5 Integrations and Observability

External providers are managed from the Integrations page (Figure 5.12): the GitHub App (installation, repository listing, webhook-driven incidents on merged pull requests); Sentry [14], Datadog [16], and PagerDuty [15], each a webhook incident source; Slack [17], with OAuth install and one-click approve/reject from a Slack message; Argo CD; and Stripe. Companion pages log webhook deliveries and probe integration health.
...
On the observability side sit performance charts, a per-organisation token cost tracker with a daily budget, parallel system-health probes (PostgreSQL, Redis, Neo4j, MinIO, Temporal), and a pgvector-backed knowledge base. The Eval Harness (Figure 5.13) runs side-by-side OpenAI-versus-Ollama comparisons across fixture incidents, persisting transcripts, tracking cost, and offering administrator-only CSV export.
...
Append-only and hash-chained, the Audit Log (Figure 5.14) builds each entry on its predecessor's hash; thus, tampering breaks the chain and becomes detectable through the verify endpoint. CSV export is also available for offline review.
